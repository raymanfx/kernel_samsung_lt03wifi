...
static void oled_detection_work(struct work_struct *work)
{
	...
	if (!oled_det_level) {
		if (lcd->oled_detection_count < 3) {
			schedule_delayed_work(&lcd->err_fg_detection, msecs_to_jiffies(10));
			lcd->oled_detection_count++;
		} else {
			dev_info(&lcd->ld->dev, "VGH IS NOT OK! LCD SMASH!!!\n");
			getnstimeofday(&ts);
			rtc_time_to_tm(ts.tv_sec, &tm);
			sprintf(name, "%s%d-%02d-%02d_%02d:%02d:%02d.log",
				"/sdcard/", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
...
static int s6evr02_set_acl(struct lcd_info *lcd)
{
	if (lcd->acl_enable) {
		if (lcd->cur_acl == 0) {
			if (lcd->bl == 0) {
				...
				s6evr02_write(lcd, SEQ_ACL_ON, ARRAY_SIZE(SEQ_ACL_ON));
		...
		case GAMMA_30CD:
			if (lcd->cur_acl != 33) {
				s6evr02_write(lcd, ACL_CUTOFF_TABLE[ACL_STATUS_33P], ACL_PARAM_SIZE);
				lcd->cur_acl = 33;
				dev_dbg(&lcd->ld->dev, "%s : cur_acl=%d\n", __func__, lcd->cur_acl);
			}
			break;
		default:
			if (lcd->cur_acl != 40) {
				s6evr02_write(lcd, ACL_CUTOFF_TABLE[ACL_STATUS_40P], ACL_PARAM_SIZE);
				lcd->cur_acl = 40;
				dev_dbg(&lcd->ld->dev, "%s : cur_acl=%d\n", __func__, lcd->cur_acl);
			}
			break;
...
static int s6evr02_ldi_init(struct lcd_info *lcd)
{
	...
	if (lcd->id[1] == 0x10) { /* for S.LSI UB(YOUM) */
		...
		msleep(20);
/*		s6evr02_write(lcd, SEQ_GAMMA_CONDITION_SET, ARRAY_SIZE(SEQ_GAMMA_CONDITION_SET));
		s6evr02_write(lcd, SEQ_GAMMA_UPDATE, ARRAY_SIZE(SEQ_GAMMA_UPDATE)); */
		s6evr02_gamma_ctl(lcd);
		s6evr02_write(lcd, SEQ_BRIGHTNESS_CONTROL_ON, ARRAY_SIZE(SEQ_BRIGHTNESS_CONTROL_ON));
		s6evr02_write(lcd, SEQ_AOR_CONTROL, ARRAY_SIZE(SEQ_AOR_CONTROL));
...
static void s6evr02_read_id(struct lcd_info *lcd, u8 *buf)
{
	...
	ret = s6evr02_read(lcd, LDI_ID_REG, LDI_ID_LEN, buf, 3);
	if (!ret) {
		lcd->connected = 0;
...
static int s6evr02_read_mtp(struct lcd_info *lcd, u8 *mtp_data)
{
	...
	ret = s6evr02_read(lcd, LDI_MTP_ADDR, LDI_MTP_LENGTH, mtp_data, 0);
...
*/
	...
	if (lcd->connected) {
		...
		if (request_irq(gpio_to_irq(GPIO_OLED_DET), oled_detection_int,
			IRQF_TRIGGER_LOW, "oled_detection", lcd))
